...
For example, if Spring Boot is using Spring Security 3.2.0-RC2, it will be resolved as 3.2.0-SNAPSHOT.
This can be also useful when you want to use dependencies from the xref:#from_maven_local[local Maven cache].

[source,]
----
./gradlew :security:security-method:build -PforceSnapshots -PfromMavenLocal=org.springframework.security
----
